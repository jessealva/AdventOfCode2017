...
public class Day2 {
    ...
    private static int[][] finalSs= new int [][]{
    ...
    public static void main(String[] args) {
        System.out.println("The checksum for first spreadsheet is: " + getSSChecksumP1(ss1)+ " but should be: 18");
        System.out.println("The checksum for first spreadsheet is: " + getSSChecksumP1(finalSs));


        System.out.println("The checksum for first spreadsheet is: " + getSSChecksumP2(ss2)+ " but should be: 9");
        System.out.println("The checksum for first spreadsheet is: " + getSSChecksumP2(finalSs));
    }

    public static int getSSChecksumP1(int[][] ss) {
        int cs = 0;
        ...
        for(int i = 0; i < ss.length ; ++i) {
            ...
            for(int j = 0; j < ss[i].length; ++j) {
                high = (ss[i][j] > high) ? ss[i][j] : high;
                low = (ss[i][j] < low) ? ss[i][j] : low;
            }
            cs += high - low;
        }
        return cs;
    ...
    public static int getSSChecksumP2(int[][] ss) {
        int cs = 0;

        for(int i = 0; i < ss.length ; ++i) {
            boolean found = false;
            for(int j = 0; j < ss[i].length; ++j) {
                for(int k = j+1 ; k < ss[i].length; ++k) {
                    int l, s;
                    if (ss[i][j] > ss[i][k]){
                        l = ss[i][j];
                        s = ss[i][k];
